...
# ===== NUEVO: Endpoint para consulta individual (Chat IA) =====

class UserProfileForChat(BaseModel):
    """Modelo para respuesta del perfil de usuario para el chat de IA."""
    user_id: str
...
@router.get("/user-profile/{user_id}", response_model=UserProfileForChat)
def get_user_profile_for_chat(
    user_id: str = Path(..., description="ID del usuario (user_id_raiz)")
):
    """
    Obtiene el perfil de riesgo de un usuario específico para el chat de IA.
    
    Este endpoint consulta directamente la base de datos analítica
    sin necesidad de tener el clustering en memoria.
    
    Útil para:
    - Contextualizar respuestas del chatbot con IA
    - Personalizar intervenciones según nivel de riesgo
    - Detectar usuarios que necesitan apoyo adicional
    """
    ...
    try:
        ...
        if row is None:
            # Usuario no tiene datos de clustering aún
            return UserProfileForChat(
                user_id=user_id,
                has_data=False,
                recommendation_context="Usuario nuevo o sin suficiente actividad para análisis. Tratar con precaución y empatía general."
            )
        
        # Construir KPIs
        kpis = {
            "reciprocidad_social": float(row.ratio_reciprocidad_social or 0),
            "dias_inactivo": float(row.dias_desde_ultima_conexion or 0),
            "mensajes_nocturnos": float(row.ratio_mensajes_nocturnos or 0),
            "apatia_perfil": float(row.indice_apatia_perfil or 0),
            "negatividad": float(row.indice_negatividad_nlp or 0),
            "participacion_comunitaria": float(row.densidad_participacion_comunitaria or 0)
        }
        
        risk_level = row.cluster_label or "BAJO_RIESGO"
        
        # Generar contexto para el prompt de IA
        context_parts = []
        
        if risk_level == "ALTO_RIESGO":
            context_parts.append("⚠️ Usuario identificado en ALTO RIESGO emocional.")
            context_parts.append("Responde con máxima empatía y cuidado.")
            context_parts.append("Considera sugerir recursos de apoyo profesional si es apropiado.")
        elif risk_level == "RIESGO_MODERADO":
            context_parts.append("Usuario con indicadores de riesgo MODERADO.")
            context_parts.append("Presta atención especial a señales de malestar.")
        else:
            context_parts.append("Usuario con perfil de bajo riesgo.")
        
        if kpis["dias_inactivo"] > 7:
            context_parts.append(f"Ha estado inactivo {int(kpis['dias_inactivo'])} días - puede estar pasando por un momento difícil.")
        
        if kpis["negatividad"] > 0.6:
            context_parts.append("Su contenido reciente muestra alta negatividad emocional.")
        
        if kpis["reciprocidad_social"] < 0.2:
            context_parts.append("Tiene bajo nivel de conexiones sociales en la plataforma.")
        
        if kpis["mensajes_nocturnos"] > 0.4:
            context_parts.append("Muestra patrones de actividad nocturna elevados.")
        
        return UserProfileForChat(
            user_id=user_id,
            risk_level=risk_level,
            severity_index=sum(kpis.values()) / len(kpis),  # Promedio simple
            kpis=kpis,
            has_data=True,
            recommendation_context=" ".join(context_parts)
        )
        
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error consultando perfil de usuario: {str(e)}"
        )
